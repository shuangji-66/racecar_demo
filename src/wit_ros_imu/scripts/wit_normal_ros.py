...
def handleSerialData(raw_data):
    ...
    if key < 11:  # 根据数据长度位的判断, 来获取对应长度数据
        ...
        if buff[1] == 0x51 :
            ...
            if checkSum(data_buff[0:10], data_buff[10]):
                longitude_imu = (hex_to_data(data_buff[2:6])[0]  // 100000.0 ) +  ((hex_to_data(data_buff[2:6])[0]  % 10000000) / 10000000.0)
                latitude_imu = (hex_to_data(data_buff[6:10])[0]  // 100000.0 ) +((hex_to_data(data_buff[6:10])[0] % 10000000) / 10000000.0)
            else:
                print('0x57 校验失败')
